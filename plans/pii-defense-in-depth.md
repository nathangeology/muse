...
### Layer 1: Pre-observe scrub (before LLM sees raw conversations)

New package: `internal/scrub/scrub.go`

A deterministic text sanitizer that runs regex-based replacement on conversation text before it hits the LLM during the observe phase. Raw conversations stay in S3 (so you can re-observe with different scrub rules later), but the LLM never sees unscrubbed data.
...
Hooks into `observeSession` in `compose.go`, scrubbing the formatted conversation before the LLM call.
...
This is part of the improve-observations plan and should ship first.
...
This is belt-and-suspenders. If the observe prompt and the learn prompt both failed to strip something, the deterministic pass catches it.
...
After a successful compose, purge processed conversations from S3. Raw conversations are only needed until they're observed. Add `PurgeProcessedConversations` to the storage client.
...
1. **Layer 2** (strip tool data in formatSession) -- ships with improve-observations plan, no new package needed.
2. **`internal/scrub` package** -- the foundation everything else depends on. Ship with thorough tests.
3. **Layer 1** (pre-observe scrub) -- immediate risk reduction, hooks into existing observe call.
...
- **No summarization gate.** An LLM-based summarization step before observe would double LLM cost per conversation. Start with deterministic scrubbing and see if it's sufficient. Add the summarization layer later if residual PII risk warrants the cost.
